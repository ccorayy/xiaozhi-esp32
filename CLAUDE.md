...
| Adres | `192.168.1.80` (WiFi "Pars", MAC `88:A2:9E:86:C9:3F`) ⚠️ **sabit IP ayrılmadı** |
| SSH | `ssh -i ~/.ssh/pi_xiaozhi pi@192.168.1.80` — anahtar bu makinede, `sudo` şifresiz |
...
- **Sabit IP yok.** `ota_url` içinde `192.168.1.80` yazılı; modem başka adres verirse cihaz sunucuyu bulamaz.
