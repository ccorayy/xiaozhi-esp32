...
| **LVGL yığını artık PSRAM'de** | 12 Eyl 2026 senkronundan sonra `CONFIG_LV_USE_BUILTIN_MALLOC=y` ve TLSF havuzu PSRAM'de: **1 MB taban** (`LV_MEM_SIZE_KILOBYTES=1024`) + `lcd_display.cc`'nin eklediği 2560 KB. Kazanç: eskiden CLIB malloc + `SPIRAM_MALLOC_ALWAYSINTERNAL=2048` yüzünden 2 KB altındaki her LVGL ayırması **dahili SRAM**'den geliyordu; dahili RAM zaten dardı. **Cihazda ölçüldü (12 Eyl 2026, Bilgi sayfası): boş dahili RAM ~75 KB → 144 KB, boş PSRAM 3976 KB.** Yani PSRAM'de ~3,9 MB oynama payı var. ⚠️ **Bedeli: tek bir ayırma 1 MB'ı geçemiyor** (TLSF `block_size_max = 1 << ceil(log2(LV_MEM_SIZE))`). Galeride ~724×724'ten büyük bir JPEG çözülemez (**henüz cihazda denenmedi**). Olursa çare board `config.json`'a `CONFIG_LV_MEM_SIZE_KILOBYTES=2048` (taban havuz 1 MB daha PSRAM yer, sınır 2 MB'a çıkar) — paylaşılan `sdkconfig.defaults.esp32s3`'e dokunma. `CONFIG_LV_CACHE_DEF_SIZE` artık 0; `SpiLcdDisplay` kurucusu açılışta `lv_image_cache_resize(2 MB)` çağırıyor, board'dan ayarlamanın anlamı yok. |
| **Taşan ekranda kaydırma** | `lv_obj_create` kaydırmayı açık getiriyor, biz `LV_OBJ_FLAG_SCROLLABLE`'ı kaldırıyorduk. İçerik ekranı aşınca **alttaki satırlara hiç erişilemiyor** ve hiçbir uyarı yok — bu hata iki kez yaşandı: önce menüde (son satır "Kısayol" görünmüyordu), sonra Bilgi sayfasında (10 satır 284 px'i ~40 px aşıyor, "Boş RAM"/MAC/Sunucu kayboldu). Doğrusu: `lv_obj_set_scroll_dir(obj, LV_DIR_VER)` + `LV_SCROLLBAR_MODE_AUTO`. LVGL yalnızca gerçekten taşanı kaydırır. ⚠️ Bedeli: taşan bir görünümde **yukarı kaydırma sohbete geçmiyor** (LVGL dikey sürüklemeyi kaydırma sayıp `LV_EVENT_GESTURE` üretmiyor); sağa kaydırma = geri ve başlıktaki geri oku çalışmaya devam ediyor. |
